...
app.get("/listings/:id", isLoggedIn, async (req, res, next) => {
  try {
    ...
    const listing = await Listing.findById(id)
      ...
      .populate({ path: "owner" }); // Mongoose will handle the conversion internally
    if (!listing) {
      req.flash("error", "Listing you requested for does'nt exist!!!");
      res.redirect("/listings");
    }
    console.log("At Line 218", listing.geometry.coordinates);
    if (listing.geometry.coordinates.length === 0) {
            try {
              let response = await geocodingClient
                ...
                .send();
                console.log("Inside try: " + response.body.features[0].geometry.coordinates);    
